...
function handle_image_upload(string $field): ?string
{
    ...
    return handle_image_upload_file($_FILES[$field]);
}

/**
 * Zpracuje JEDEN nahraný soubor (pole se strukturou jako položka $_FILES).
 * Použití pro vícenásobný upload, kde se prochází pole souborů ručně.
 * Vrací cestu (images/uploads/...) nebo null, když nic nebylo nahráno.
 */
function handle_image_upload_file(array $file): ?string
{
...
/**
 * Sanitizace HTML z editoru Quill - povolí jen bezpečné formátovací značky
 * (tučné, kurzíva, podtržení, nadpisy, seznamy, odkazy, odstavce, zlomy).
 * Odstraní <script>, <style>, event atributy a nebezpečné odkazy.
 */
function clean_html(string $html): string
{
    ...
    // povolené značky
    $allowedTags = '<p><br><strong><b><em><i><u><h1><h2><h3><h4><ul><ol><li><a><span>';
    $html = strip_tags($html, $allowedTags);

    // odstraníme všechny on* atributy (onclick apod.) a javascript: odkazy
    $html = preg_replace('/\son\w+\s*=\s*"[^"]*"/i', '', $html);
    $html = preg_replace("/\son\w+\s*=\s*'[^']*'/i", '', $html);
    $html = preg_replace('/javascript\s*:/i', '', $html);
    // style atributy raději pryč (Quill je běžně nepoužívá u základního formátování)
