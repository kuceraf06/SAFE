    ...
    <?php include '../../../../skeleton/headerAdmin.php'?>
        ...
        <!-- Formulář pro přidání nové události -->
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $targetDir = "../../../../images/";  // Cesta pro uložení obrázku na serveru
                $imagePaths = [];
                ...
                if (!empty($_FILES['eventImages']['name'][0])) {
                    foreach ($_FILES['eventImages']['name'] as $index => $fileName) {
                        ...
                        if (move_uploaded_file($_FILES['eventImages']['tmp_name'][$index], $targetFile)) {
                            // Ukládání relativní cesty k obrázku
                            $imagePaths[] = "images/" . basename($fileName);
                        }
                    }
                }

                // Uložení cesty k obrázkům jako JSON do databáze
                $imagesJson = json_encode($imagePaths);
                $stmt = $conn->prepare("INSERT INTO pastevents (title, description, images) VALUES (?, ?, ?)");
